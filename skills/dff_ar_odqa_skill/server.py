...
ODQA_SERVICE_URL = os.getenv("ODQA_SERVICE_URL")
assert ODQA_SERVICE_URL
...
while True:
    result = containers.is_container_running(ODQA_SERVICE_URL)
    if result:
        logger.info(f"ODQA_SERVICE_URL: {ODQA_SERVICE_URL} is ready")
        break
    else:
        time.sleep(5)
        continue
